...
supported_video_ext = ('.avi', '.mp4', '.MOV')
...
def main():
    ...
    # Extract frames from given video file
    if args.video:
        video_basename = osp.basename(args.video).split('.')[0]  # Setup video extractor for given video file
        video_ext = osp.splitext(args.video)[-1] # Check video file extension
        if video_ext not in supported_video_ext: 
            raise ValueError("Not supported video format: {}".format(video_ext))

        output_dir = osp.join(args.output_root, '{}_frames'.format(video_basename))   # Set extracted frames output directory
        # Set up video extractor for given video file 
        ...
        for r, d, f in walk(args.dir):
            for file in f: 
                ...
                if file_ext in supported_video_ext:
                    video_list.append([osp.join(osp.relpath(r, args.dir), file), osp.join(osp.relpath(r, args.dir), "{}_frames".format(file_basename))])
